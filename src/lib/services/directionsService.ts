...
export async function createDirections(directions: Direction[]): Promise<Direction[]> {
    try {
        const results = await db.insert(directionsTable).values(directions).returning();
        return results
    } catch (e) {
        console.error(e)
        throw e
